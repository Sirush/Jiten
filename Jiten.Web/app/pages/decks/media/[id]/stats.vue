<script setup lang="ts">
  ...
  const {
    ...
    error: deckError,
    ready: deckReady,
  } = await useApiFetchPaginated<DeckDetail>(`media-deck/${deckId.value}/detail`);
  ...
  } = await useApiFetch<DeckDifficultyDto>(`media-deck/${deckId.value}/difficulty`);
...
<template>
  <div class="flex flex-col gap-4">
    <DeckBreadcrumb :deck="deckResponse?.data?.mainDeck" :parent-deck="deckResponse?.data?.parentDeck" current="Statistics" />
    ...
    <Card v-else-if="stats">
      ...
      <template #content>
        <LazyCoverageChart v-if="curveData" class="hidden sm:block" :curve-data="curveData" />
