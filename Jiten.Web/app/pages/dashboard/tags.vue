<script setup lang="ts">
  ...
  function addNewMappingRow() {
    const lastProvider = newMappingRows.value.length > 0 ? newMappingRows.value[newMappingRows.value.length - 1].provider : LinkType.Anilist;
...
<template>
  <div class="container mx-auto p-4">
      ...
      <Column header="Actions" style="width: 250px">
        <template #body="slotProps">
          <div class="flex gap-2">
            <Button icon="pi pi-pencil" size="small" severity="secondary" @click="openEditDialog(slotProps.data)" />
            <Button icon="pi pi-chart-bar" size="small" severity="info" @click="viewUsage(slotProps.data.tagId)" />
            <Button v-tooltip.top="'Manage Mappings'" icon="pi pi-link" size="small" severity="info" @click="openMappingsDialog(slotProps.data)" />
            <Button icon="pi pi-trash" size="small" severity="danger" @click="confirmDelete(slotProps.data)" />
          </div>
        </template>
      </Column>
    </DataTable>
    ...
    <!-- Create/Edit Dialog -->
    <Dialog v-model:visible="showDialog" :header="dialogMode === 'create' ? 'Create Tag' : 'Edit Tag'" :modal="true" class="w-full md:w-1/2">
      <div class="p-fluid">
        <div class="mb-4">
          <label for="tagName" class="block text-sm font-medium mb-2">Tag Name</label>
          <InputText id="tagName" v-model="tagName" placeholder="Enter tag name" class="w-full" @keyup.enter="saveTag" />
        </div>
      </div>
      <template #footer>
    ...
    <!-- Usage Dialog -->
    <Dialog v-model:visible="showUsageDialog" header="Tag Usage Statistics" :modal="true" class="w-full md:w-1/2">
    ...
    >
      <div class="space-y-6">
        ...
        <div>
          ...
          <DataTable v-else-if="tagMappings.length > 0" :value="tagMappings" class="shadow-sm" striped-rows>
          ...
          <div v-else class="text-center py-8 text-gray-500 dark:text-gray-400">No existing mappings for this tag</div>
        </div>
        ...
        <div>
          ...
          <div class="space-y-3">
            <div v-for="row in newMappingRows" :key="row.id" class="grid grid-cols-12 gap-2 items-end">
              <div class="col-span-12 md:col-span-4">
                <label class="block text-sm font-medium mb-1">Provider</label>
                <Select v-model="row.provider" :options="providers" option-label="label" option-value="value" class="w-full" :disabled="row.isSaving" />
              </div>
              ...
              <div class="col-span-12 md:col-span-3 flex gap-2">
                ...
                />
                <Button icon="pi pi-trash" severity="danger" :disabled="row.isSaving" @click="removeNewMappingRow(row.id)" />
              </div>
            </div>
          </div>
          <div class="flex justify-end mt-4">
            <Button label="Add Row" icon="pi pi-plus" severity="secondary" @click="addNewMappingRow" />
          </div>
        </div>
      </div>
