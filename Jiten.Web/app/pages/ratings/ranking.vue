<script setup lang="ts">
  ...
  const loadBitmap = createBitmapLoader();
  ...
  async function exportPng() {
    ...
    try {
      ...
      const [bitmaps] = await Promise.all([Promise.all(decks.map((d) => loadBitmap(d.coverUrl || NOCOVER))), document.fonts.ready]);
      const logoBitmap = await loadBitmap('/favicon-96x96.png');
      ...
      if (logoBitmap) drawCoverImage(ctx, logoBitmap, brandRight - brandW - logoGap - logoSize, y + 4, logoSize, logoSize, pal.brand);
      ...
      groups.forEach((group, index) => {
        ...
        for (const deck of group.decks) {
          drawCoverImage(ctx, bmpByDeck.get(deck.id) ?? null, rowX, ry, COVER_W, ROW_H, pal.sub);
          ctx.fillStyle = pal.text;
          ctx.font = `500 15px ${FONT}`;
          const tx = rowX + COVER_W + 10;
          ctx.fillText(fitCanvasText(ctx, deckTitle(deck), W - PAD - tx), tx, ry + ROW_H / 2 + 5);
          ry += ROW_H + ROW_GAP;
        }
        y += rh + RANK_GAP;
      });
      ...
      await saveCanvasPng(canvas, `jiten-ranking-${slug}.png`, 'Difficulty Ranking');
    } catch (e) {
...
<template>
  <div class="p-2 md:p-4 overflow-hidden">
    ...
    <div v-else-if="sections.length === 0" class="text-center text-muted-color py-12">Complete at least two titles to start ranking.</div>
    ...
    <div v-else>
      ...
      <div v-if="tierQuality" class="mb-3 flex items-center gap-3">
        <ProgressBar :value="tierQuality.progress" :show-value="false" style="height: 6px; width: 120px" />
      ...
      <div v-if="activeSection" class="grid grid-cols-1 lg:grid-cols-[minmax(0,340px)_1fr] gap-4">
        ...
        <Card class="min-w-0" data-drop-unranked :class="{ 'is-drop-active': dragOver?.kind === 'unranked' }">
          <template #content>
            ...
            <div v-if="activeSection.unranked.length === 0" class="text-sm text-muted-color">Everything in this group is ranked.</div>
            <div v-else class="flex flex-col gap-2">
              <div v-for="deck in activeSection.unranked" :key="deck.id" class="deck-pill" @pointerdown="onPointerDown(deck, null, $event)">
        ...
        <div class="min-w-0">
          <div
            ...
            class="flex items-center gap-2 mb-2 text-xs font-semibold uppercase tracking-wide"
            :style="{ color: rankColor(0, displayGroups.length) }"
          >
            <i class="pi pi-arrow-up text-[0.6rem]" />
            Hardest
          </div>
          <div
            v-if="displayGroups.length === 0"
          ...
          <div v-else class="flex flex-col" :class="{ 'is-dragging': draggingDeck }">
            <div v-for="(group, index) in displayGroups" :key="group.id">
              <div class="drop-gap" :class="{ 'is-active': dragOver?.kind === 'gap' && dragOver.index === index }" :data-drop-gap="index">
              ...
              >
                <template #content>
                  <div class="flex items-center justify-between mb-2">
                    <div class="text-xs uppercase tracking-wide font-semibold" :style="{ color: rankColor(index, displayGroups.length) }">
                      Rank {{ index + 1 }}
                    </div>
                    <Transition name="fade-label">
                      <div v-if="draggingDeck && dragOver?.kind === 'group' && dragOver.groupId === group.id" class="drop-tie-label">Drop to tie</div>
                    ...
                    >
                      ...
                      <Tooltip :content="`↑↑ New rank above\n↑ Move to rank above\n↓ Move to rank below\n↓↓ New rank below`" block>
                        <div class="rank-actions" @pointerdown.stop>
                          ...
                          <button
                            class="rank-btn"
                            :disabled="isUpdating || index === displayGroups.length - 1"
                            @click.stop="mergeWithRank(deck.id, displayGroups[index + 1].id)"
                          >
          ...
          <div
            v-if="displayGroups.length > 0"
            class="flex items-center gap-2 mt-2 text-xs font-semibold uppercase tracking-wide"
            :style="{ color: rankColor(displayGroups.length - 1, displayGroups.length) }"
          >
            <i class="pi pi-arrow-down text-[0.6rem]" />
            Easiest
          </div>
        </div>
      </div>
